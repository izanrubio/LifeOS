...
import DayFeeling from '@/components/DayFeeling'
...
export default function TodayPage() {
  ...
  const updateDayFeeling = async (value: number | null) => {
    if (!user || !dailyEntry) return

    await supabase
      .from('daily_entries')
      .update({ day_feeling: value })
      .eq('id', dailyEntry.id)

    setDailyEntry({ ...dailyEntry, day_feeling: value })
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-950 to-emerald-900 flex flex-col">
      ...
      <div className="flex-1 px-8 py-12">
        <div className="max-w-2xl mx-auto">
          ...
          {/* Day Feeling */}
          <DayFeeling 
            value={dailyEntry?.day_feeling ?? null}
            onChange={updateDayFeeling}
          />
        </div>
      </div>
